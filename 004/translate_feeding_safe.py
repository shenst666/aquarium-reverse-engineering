"""
安全翻译 feeding.min.js：只动显示字符串，不动代码逻辑值
"""
import re
...
# ============================================================
# 安全规则：
# ✅ 翻译：name, foods[], avoid[], notes, perFeed, total, UI 标签
# ❌ 不动：category, diet, id, 属性名, stage 值, 比较字符串
# ============================================================
...
SAFE = [
    ...
    ("Sinking pellets that reach them", "能沉到底的颗粒"),
    ("fresh vegetables (courgette, spinach)", "新鲜蔬菜（西葫芦、菠菜）"),
    ("catfish sinking pellets", "鲶鱼沉底颗粒"),
...
print(f'替换: {count}/{len(SAFE)}')

with open(fp, 'w', encoding='utf-8') as f:
    f.write(js)

print('feeding.min.js 安全翻译完成')

# 验证关键代码未受损
assert "s.category === 'freshwater'" in js, "❌ freshwater 比较被破坏!"
assert "s.category === 'marine'" in js, "❌ marine 比较被破坏!"
assert "adultMode" in js, "❌ adultMode 属性被破坏!"
assert "juvenileMode" in js, "❌ juvenileMode 属性被破坏!"
assert "'freshwater'" in js, "❌ freshwater 值丢失!"
print("✅ 所有代码关键值完整")
